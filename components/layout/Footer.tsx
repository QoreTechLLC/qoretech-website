...
export default function Footer() {
  ...
  return (
    <footer className="border-t border-neon-500/[0.06]" aria-label="Site footer">
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        <div className="py-16 grid grid-cols-2 md:grid-cols-5 gap-10">
          ...
          {Object.entries(links).map(([section, items]) => (
            <div key={section}>
              <h3 className="text-[10px] font-semibold tracking-widest uppercase text-frost-600 mb-5">{section}</h3>
              <ul className="space-y-3">
                {items.map((item) => (
                  <li key={item}>
                    <a href="#" className="text-sm text-frost-500 hover:text-frost-100 transition-colors">{item}</a>
                  </li>
                ))}
              </ul>
            </div>
        ...
        <div className="py-6 border-t border-neon-500/[0.05] flex flex-col sm:flex-row items-center justify-between gap-4">
          <p className="text-xs text-frost-700">© {year} QoreTech. All rights reserved.</p>
          <div className="flex items-center gap-1.5">
            <span className="w-1.5 h-1.5 rounded-full bg-neon-500 animate-pulse" />
            <span className="text-xs text-frost-600">All systems operational</span>
          </div>
        </div>
      </div>
    </footer>
