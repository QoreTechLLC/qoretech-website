...
const floatingCards = [
  { icon: <Zap className="w-4 h-4 text-neon-500" />,   label: 'Deploy complete', sublabel: 'Production · 1.2s',   dotClass: 'bg-neon-500' },
  { icon: <Shield className="w-4 h-4 text-frost-200" />, label: 'AI Integration',   sublabel: 'OpenAI · Active',    dotClass: 'bg-frost-300' },
  { icon: <Layers className="w-4 h-4 text-neon-400" />, label: 'Infra scaled',     sublabel: '99.99% uptime',      dotClass: 'bg-neon-400' },
]

const stagger = {
  container: { hidden: {}, show: { transition: { staggerChildren: 0.1, delayChildren: 0.3 } } },
  item: { hidden: { opacity: 0, y: 24 }, show: { opacity: 1, y: 0, transition: { duration: 0.7, ease: [0.21,0.47,0.32,0.98] } } },
}

export default function Hero() {
  return (
    <section className="relative min-h-screen flex flex-col items-center justify-center overflow-hidden pt-20 pb-24 grid-bg" aria-label="Hero">
      {/* Glow orbs */}
      <GlowOrb color="neon"  size="xl" className="-top-40 -left-40 opacity-[0.08]" />
      <GlowOrb color="neon"  size="lg" className="top-1/3 -right-32 opacity-[0.06]" />
      <GlowOrb color="frost" size="md" className="bottom-20 left-1/3 opacity-[0.05]" />

      {/* Scan line effect */}
      <div aria-hidden="true" className="absolute inset-0 overflow-hidden pointer-events-none">
        <div className="absolute inset-x-0 h-px bg-gradient-to-r from-transparent via-neon-500/30 to-transparent animate-scan" />
      </div>

      {/* Vignette */}
      <div aria-hidden="true" className="absolute inset-0 bg-gradient-radial from-transparent via-transparent to-navy-950/90 pointer-events-none" />
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 lg:px-8 flex flex-col items-center text-center">
        ...
        {/* Headline */}
        <motion.div variants={stagger.container} initial="hidden" animate="show" className="space-y-2 mb-8">
          <motion.h1 variants={stagger.item} className="text-[clamp(2.6rem,6vw,5rem)] font-semibold tracking-tight leading-[1.04] text-frost-100">
            Scalable technology
          </motion.h1>
          <motion.h1 variants={stagger.item} className="text-[clamp(2.6rem,6vw,5rem)] font-semibold tracking-tight leading-[1.04] text-gradient">
        ...
        >
          <div className="relative glass rounded-3xl border border-neon-500/[0.09] p-1 shadow-[0_32px_96px_rgba(0,0,0,0.7),0_0_0_1px_rgba(0,217,126,0.05)]">
            ...
            <div className="flex items-center gap-2 px-4 py-3 border-b border-neon-500/[0.07]">
              <div className="flex gap-1.5">
                {['bg-navy-700','bg-navy-700','bg-navy-700'].map((c,i) => <div key={i} className={`w-3 h-3 rounded-full ${c}`} />)}
              </div>
            ...
            <div className="p-6">
              <div className="grid grid-cols-3 gap-3 mb-4">
                {[
                  ...
                  { label: 'Deployments', value: '347',    color: 'text-frost-100' },
                  { label: 'AI Requests', value: '1.2M',   color: 'text-neon-400'  },
                ].map((s) => (
                  <div key={s.label} className="glass rounded-xl p-4 text-center border border-neon-500/[0.06]">
              ...
              <div className="glass rounded-xl p-4 border border-neon-500/[0.06]">
                ...
                <p className="text-[10px] text-frost-600 mt-3">Request volume — last 12 months</p>
              </div>
            </div>
          </div>
          ...
          {floatingCards.map((card, i) => (
            ...
            >
              ...
              <div>
                <p className="text-xs font-medium text-frost-100 leading-none">{card.label}</p>
                <p className="text-[10px] mt-0.5 text-neon-500">{card.sublabel}</p>
              </div>
              <div className={`w-1.5 h-1.5 rounded-full ml-1 ${card.dotClass} animate-pulse`} />
            </motion.div>
          ))}
        </motion.div>
